...
# Create an empty vector to store the silhouette widths
silhouette_widths <- numeric(length(cluster_range))
...
for (k in cluster_range) {
  ...
  silhouette_width <- cluster.stats(DF, kmeans_result$cluster)$avg.silwidth
  
  # Store the silhouette width for this number of clusters
  silhouette_widths[k - cluster_range[1] + 1] <- silhouette_width
}
# 21:23 start
# Plot the average silhouette width for each number of clusters
plot(cluster_range, silhouette_widths, type = "b", 
     xlab = "Number of Clusters", ylab = "Average Silhouette Width",
     main = "Average Silhouette Width for Different Numbers of Clusters")

# Find the number of clusters with the highest silhouette width
optimal_clusters <- cluster_range[which.max(silhouette_widths)]

# Add points and text to the plot to highlight the optimal number of clusters
points(optimal_clusters, silhouette_widths[optimal_clusters - cluster_range[1] + 1], col = "red", pch = 19)
text(optimal_clusters, silhouette_widths[optimal_clusters - cluster_range[1] + 1], labels = optimal_clusters, pos = 3, col = "red")
