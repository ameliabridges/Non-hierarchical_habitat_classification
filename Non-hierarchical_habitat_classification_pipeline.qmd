...
results_layers_folder<-paste0(wd, "/Results")
```
...
The productivity layer we're using comes from the NEMO-MEDUSA model. Authors have uploaded the netCDF version on Zenodo, [here](https://zenodo.org/record/6513616#.Yn6TGx1BzIU). However for ease, the TIFF version is in the InputLayers folder. Let's read it in. 
...
```{r}
terra::writeRaster(slo, filename=(paste0(input_layers_folder, "/GEBCO2020_Slo_10km_MOL.tif")), overwrite=TRUE, format="GTiff")
```
